...
export default function CollectionHeroes() {
  ...
  const { heroesList, isLoadingList, errorListResp } = useHeroesList();

  const { value, setSearchHandler } = useInputHeroes();

  const { filterList, handleFilterClick, currentFilterAttr, currentFilterComp } = useFilterCollectionHeroes(value, heroesList);
  ...
  if (errorListResp) {
    return <div>Error: {errorListResp.message}</div>;
  }
  ...
  return (
      <section className={styles["heroes"]}>
        ...
        {filterList.length === 0 ? (
          ...
          <HeroesList heroesData={filterList} currentFilterAttr={currentFilterAttr} currentFilterComp={currentFilterComp} searchValue={value} />
          </div>
        )}
      </section>
  );
